...
def parse_tenencias(pdf_bytes: bytes) -> dict:
    """
    Extrae del PDF de tenencias:
      - periodo (ej. "Abril 2026")
      - total_nominal_mn (unidades MN nominal)
      - tenencias_por_tipo: % globales por inversor
      - pct_por_bono: { "12AGO2026": {"Offshores": 7.0, "Banks": 63.42, ...}, ... }
      - evolucion: lista de {fecha, Offshores, PFs, Banks, Insurance, Others}
    """
    import pdfplumber
    ...
    # ── Total MN Nominal (unidades) ───────────────────────────────
    # Aparece como "NNN NNN NNN Unidades" después de "Moneda Nacional Nominal"
    m = re.search(
        r"Moneda Nacional Nominal\s+([\d\s]+)\s+Unidades",
        text
    )
    if m:
        units = int(m.group(1).replace(" ", ""))
        result["total_nominal_mn"] = round(units / 1000)  # miles → MM (1 unidad = S/1,000)
    else:
        result["total_nominal_mn"] = 0

    # ── % Globales por tipo de inversor ───────────────────────────
    # El bloque "Bonos Soberanos en Moneda Nacional Nominal" tiene los % globales
    # Patron: "AFPs\n8.32%\nBancos\n32.73%\n..."
    # Buscamos el bloque entre "Moneda Nacional Nominal" y "Moneda Nacional Indexada"
    block_mn = re.search(
        r"Moneda Nacional Nominal(.*?)Moneda Nacional Indexada",
        text, re.S
    )
    block_text = block_mn.group(1) if block_mn else text

    tenencias = {}
    for nombre_mef, key in INVERSOR_MAP.items():
        pattern = re.escape(nombre_mef) + r"[\s\n]+([\d.]+)%"
        m = re.search(pattern, block_text, re.I)
        if m:
            tenencias[key] = float(m.group(1))
    ...
    # Estrategia: buscar cada bono objetivo y extraer el bloque de % que lo precede

    pct_por_bono = {}

    for bono in BONOS_OBJETIVO:
        # Buscar el bono en el texto
        bono_escaped = re.escape(bono)
        # El bloque de % termina justo antes de "Bonos Soberanos XXXXXYYYYY"
        # y puede empezar después del bono anterior
        pattern = (
            r"((?:(?:AFPs|Bancos|Seguros|No residentes|Fondos p[úu]blicos|"
            r"Fondos privados|Otros|Personas\s*naturales)[\s\n]+[\d.]+%[\s\n]*)+)"
            r"Bonos Soberanos\s+" + bono_escaped
        )
        ...
        inv_pct = {}
        for nombre_mef, key in INVERSOR_MAP.items():
            p = re.escape(nombre_mef) + r"[\s\n]+([\d.]+)%"
            found = re.search(p, block, re.I)
            if found:
    ...
    # ── Evolución histórica (% globales mensuales) ────────────────
    # El gráfico de evolución tiene una fila de fechas tipo "May-25 Jun-25 ..."
    # y filas de valores por categoría. Extraemos los % de AFPs, Bancos,
    # No residentes, Seguros del texto.
    #
    # Formato en el PDF:
    # "9.26% 10.13% ... 8.16%\n33.38%\n31.20%...32.14%\n42.85%\n44.53%...45.25%\n
    #  0%\n...\nMay-25 Jun-25 Jul-25 ..."
    #
    # Mejor estrategia: extraer las fechas y los % flotantes cercanos.
